...
// Define a function to create a leaderboard
module.exports.leaderBoard = async function leaderBoard(req, res) {
  try {
    // Find all users in descending order of points
    const users = await userModel.find().sort({ points: -1 });

    // Return a success response with the users array
    res.status(200).json({
    ...
    res.status(400).json({
      message: `Failed to create leaderboard: ${error.message}`,
    });
  }
};
